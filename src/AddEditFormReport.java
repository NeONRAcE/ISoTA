				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent arg0)
					{
						if(validateReports())
						{
							try
							{
							Date d = TextParser.parseDate(tfOperDate.getText(), "Дата операции");
							Float overpayment = TextParser.parseFloat(tfOverpayment.getText(), "Переплата");
							Float paid = TextParser.parseFloat(tfPaid.getText(), "Оплачено");
							Float returned = TextParser.parseFloat(tfReturned.getText(), "Возвращено");
							Float sum = TextParser.parseFloat(tfSum.getText(), "Сумма");
							
							ReportModel rm = new ReportModel();
							rm.setOperationDate(d);
							rm.setKod(tfOperCode.getText().toString());
							rm.setOverpayment(overpayment);
							rm.setPaid(paid);
							rm.setReturned(returned);
							rm.setSum(sum);
							rm.save();
							}
							catch (Exception e)
